...
class BabelNode:
    ...
    def process_leak(cls, raw_content: str) -> dict:
        ...
        try:
            ...
            logger.info(
                f"[BABEL NODE] Detected languages: {langs} | Entities found: {len(entities['emails'])} emails, {len(entities['btc_wallets'])} BTC"
            )
            ...
            return {
                ...
                "needs_translation": any(
                    lang for lang in langs if "Latin" not in lang
                ),  # Flag per future translation pipeline
            }
