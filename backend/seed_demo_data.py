...
async def seed_demo():
    ...
    async with async_session() as db:
        with Progress(
            ...
            task2 = progress.add_task("[yellow]Seeding High-Profile Identities (VIP & Standard)...", total=len(VIP_IDENTITIES)+len(STANDARD_IDENTITIES))
            ...
            await db.commit()
            progress.update(task3, description=f"[green]Seeded {leak_count} individual node connections to the Topology Matrix.")
