...
async def _login(client, email="csrf-admin@naso.local", password="Csrf$ecure99"):
...
async def test_bearer_only_post_skips_csrf(client, admin_user):
    ...
    login = await client.post(
        "/auth/login",
        data={"username": "csrf-admin@naso.local", "password": "Csrf$ecure99"},
    )
