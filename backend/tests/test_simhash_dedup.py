...
SAMPLE = (
    "password leak for acme corp, 14000 records, including emails and hashed passwords from the forum breach of 2023"
)


async def _count(db, tenant_id):
    rows = (await db.execute(select(LeakHit).where(LeakHit.tenant_id == tenant_id))).scalars().all()
    return len(rows)


class TestFingerprintPopulation:
    async def test_normalized_content_and_simhash_are_set(self, corr_db, tenant):
        leak = await ingest_leak(corr_db, tenant_id=tenant.id, source="github", content=SAMPLE, severity_score=70)
...
class TestNearDup:
    async def test_whitespace_variant_dedups(self, corr_db, tenant):
        ...
        assert await _count(corr_db, tenant.id) == 1, "whitespace/casing variants must be recognised as near-duplicates"

    async def test_near_dup_bumps_severity_when_higher(self, corr_db, tenant):
        first = await ingest_leak(corr_db, tenant_id=tenant.id, source="github", content=SAMPLE, severity_score=50)
        variant = SAMPLE.upper()
        second = await ingest_leak(corr_db, tenant_id=tenant.id, source="github", content=variant, severity_score=80)
    ...
    async def test_near_dup_does_not_downgrade_severity(self, corr_db, tenant):
        first = await ingest_leak(corr_db, tenant_id=tenant.id, source="github", content=SAMPLE, severity_score=80)
        await ingest_leak(corr_db, tenant_id=tenant.id, source="github", content=SAMPLE.upper(), severity_score=30)
        await corr_db.refresh(first)
        assert first.severity_score == 80
