...
from ...shared.schemas import Tenant as TenantSchema, TenantCreate
...
async def create_tenant(tenant: TenantCreate, db: AsyncSession = Depends(get_db), current_user = Depends(check_admin)):
    ...
    await AuditLogger.log(
        ...
        action="CREATE_TENANT", resource_type="tenant", resource_id=db_tenant.id,
...
async def delete_tenant(tenant_id: str, db: AsyncSession = Depends(get_db), current_user = Depends(check_admin)):
    ...
    await AuditLogger.log(
        db, user_id=current_user.id, tenant_id=current_user.tenant_id,
        action="DELETE_TENANT_TRIGGERED", resource_type="tenant", resource_id=tenant_id
    )
    await db.commit()
