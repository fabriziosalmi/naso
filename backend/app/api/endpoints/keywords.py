...
from ...schemas import Keyword as KeywordSchema, KeywordCreate
from ..deps import get_current_user

router = APIRouter()

@router.post("/", response_model=KeywordSchema)
async def create_keyword(keyword: KeywordCreate, db: AsyncSession = Depends(get_db), current_user = Depends(get_current_user)):
