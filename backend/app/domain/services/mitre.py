...
from ...models import MitreTechnique, LeakHit, mitre_leaks
...
class MitreMappingService:
    ...
    async def map_leak_to_ttp(cls, db: AsyncSession, leak_id: str, content: str):
        ...
        result = await db.execute(
            select(MitreTechnique).where(MitreTechnique.id.in_(list(technique_ids)))
        )
        ...
        for tech in existing_techniques:
            ...
            check = await db.execute(
                select(mitre_leaks).where(
                    mitre_leaks.c.mitre_id == tech.id,
                    mitre_leaks.c.leak_id == leak_id
                )
            )
            if not check.first():
                await db.execute(
                    mitre_leaks.insert().values(mitre_id=tech.id, leak_id=leak_id)
                )
