...
class IdentityCorrelationService:
    ...
    @classmethod
    async def correlate_leak(cls, db: AsyncSession, leak_id: str, content: str, tenant_id: str, screenshot_path: str = None):
        ...
        for email in emails:
            ...
            assoc_check = await db.execute(
                select(identity_leaks).where(
                    identity_leaks.c.identity_id == identity.id,
                    identity_leaks.c.leak_id == leak_id
                )
            )
            if not assoc_check.first():
                await db.execute(
                    identity_leaks.insert().values(identity_id=identity.id, leak_id=leak_id)
                )
        ...
        if leak:
            ...
            if leak.severity_score >= severity_threshold:
                ...
                # Trigger Webhooks (U)
                await WebhookService.trigger_critical_leak(db, tenant_id, {
                    "id": leak.id,
                    "source": leak.source,
