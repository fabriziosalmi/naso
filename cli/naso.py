...
def monitor():
    ...
    layout["header"].update(Panel("[bold cyan]NASO LIVE MONITOR - PERFORMANCE & FORENSICS[/bold cyan]", border_style="blue"))
    
    with Live(layout, refresh_per_second=4, screen=True):
        while True:
            layout["body"].update(generate_benchmark_table())
            layout["footer"].update(Text(f"System Epoch: {time.time()} | Integrity: 100% | Auth: ROOT", style="dim italic", justify="center"))
            smart_sleep(0.25)
            if NASO_CI:
